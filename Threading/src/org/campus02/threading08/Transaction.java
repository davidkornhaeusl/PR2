...
public class Transaction {
	...
	public void doTransaction()
	{
		...
		{
			System.out.println(Thread.currentThread().getId() + " > " + accountCreditor.getAccountHolder() + " lock 1 succeeded, waiting for " + accountDepitor.getAccountHolder());
			accountCreditor.credit(amount);
			
			synchronized(accountDepitor)
			{
				System.out.println(Thread.currentThread().getId() + " > " + accountDepitor.getAccountHolder() + "  lock 2 succeeded");
